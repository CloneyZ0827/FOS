        ...
        <div class="col-lg-4 mb-4">
            <div class="card shadow-sm">
                ...
                <div class="card-footer text-center" style="background-color: rgba(216, 188, 151);">
                    <button type="submit" name="makePayment" class="btn btn-success" 
                            style="background-color: #3a3a3a; color: #d8bc97; border: none;" 
                            id="makePaymentButton" disabled> 
                        Make Payment 
                    </button>
                    </form>
                </div>
...
<script>
    ...
    // Get references to the payment method radio buttons and the submit button
    const paymentMethods = document.querySelectorAll('input[name="paymentMethod"]');
    const makePaymentButton = document.getElementById('makePaymentButton');

    // Function to enable or disable the button based on payment method selection
    function updateButtonState() {
        ...
        // Enable the button if a payment method is selected, otherwise disable it
        makePaymentButton.disabled = !isAnyMethodSelected;
    }

    // Add event listeners to all payment method radio buttons
    paymentMethods.forEach(method => {
        method.addEventListener('change', updateButtonState);
    });
